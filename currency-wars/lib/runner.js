...
exports.STRATEGY_REFRESH_POINTS = void 0;
exports.evaluateCombinedRound = evaluateCombinedRound;
exports.runCurrencyWars = runCurrencyWars;
// runner —— 货币战争自动运行核心（纯逻辑，不碰 Electron）。
// 宿主能力（截图/识别/键鼠等）全部经 tools 注入；插件化后由 ctx.services.gamebot 提供。
const text_matcher_1 = require("./text-matcher");
const image_detectors_1 = require("./image-detectors");
function evaluateCombinedRound(settings, targetSatisfied, blockedHit) {
...
const OPENING_ADVANCE = { x: 1660 / 1920, y: 965 / 1080 };
...
const INVESTMENT_FALLBACK_POINTS = [
...
const DIALOG = { x: 0.2, y: 0.04, width: 0.7, height: 0.65 };
const ROLE_TITLE = { x: 0.34, y: 0.03, width: 0.42, height: 0.12 };
const BATTLE_BUTTON_REGION = { x: 0.84, y: 0.62, width: 0.16, height: 0.22 };
const AUTO_BATTLE_REGION = { x: 0, y: 903 / 1080, width: 144 / 1920, height: 120 / 1080 };
const STRATEGY_REGION = { x: 0, y: 0.08, width: 1, height: 0.66 };
const STRATEGY_SCREEN_ALIASES = ["请选择投资策略", "刷新次数", "返回备战界面"];
const CONTINUE_ALIASES = ["点击空白处继续", "下一步", "下一页", "继续挑战", "前往结算", "确认"];
exports.STRATEGY_REFRESH_POINTS = [
...
function pointIn(target, point) {
    return {
        x: target.left + Math.round(target.width * point.x),
        y: target.top + Math.round(target.height * point.y),
    };
}
function findItem(items, aliases, fuzzyScore) {
    for (const alias of aliases) {
        const match = items.find((item) => (0, text_matcher_1.fuzzyContains)(item.text, alias, fuzzyScore));
        if (match)
            return match;
    }
    return null;
}
function findItemWithAlias(items, aliases, fuzzyScore) {
    for (const alias of aliases) {
        const item = items.find((candidate) => (0, text_matcher_1.fuzzyContains)(candidate.text, alias, fuzzyScore));
        if (item)
            return { item, alias };
    }
    return null;
}
function chooseSafeInvestment(items, fuzzyScore) {
    const blocked = new Set();
    for (const item of items) {
        if (!SPECIAL_INVESTMENT_BLACKLIST.some((word) => (0, text_matcher_1.fuzzyContains)(item.text, word, Math.max(76, fuzzyScore))))
            continue;
...
async function runCurrencyWars(ctx) {
    ...
    const checkStopped = () => {
        if (signal.aborted)
            throw new StopError("已中止");
    };
    const delay = async (ms) => { checkStopped(); await tools.delay(ms); checkStopped(); };
    const progress = (desc) => ctx.onProgress?.({
    ...
    const clickText = async (aliases, fallback, region = FULL, timeoutMs = 8000) => {
        ...
        if (fallback)
            await clickRatio(fallback);
        return false;
    };
    const clickRecognized = async (capture, item) => {
    ...
    const captureOnly = async (region = FULL) => tools.capture(await getWindow(), region);
    const handleRoleChoice = async (current) => {
        const target = await getWindow();
        const inTitle = (item) => {
        ...
        const hasTitle = (aliases) => {
            const found = findItem(current.result.items, aliases, settings.buttonFuzzyScore);
            return found && inTitle(found);
        };
        if (hasTitle(["选择伙伴"])) {
            await clickRatio({ x: 0.545, y: 0.277 });
            await delay(200);
            await clickRatio({ x: 0.777, y: 0.551 });
            await delay(500);
            return true;
        }
        if (hasTitle(["祈愿试炼"])) {
            await clickRatio(Math.random() < 0.5 ? { x: 0.357, y: 0.33 } : { x: 0.734, y: 0.33 });
            await delay(200);
            await clickRatio({ x: 0.78, y: 0.593 });
            await delay(500);
            return true;
        }
        if (hasTitle(["盛会之星"])) {
            await clickRatio(Math.random() < 0.5 ? { x: 0.485, y: 0.255 } : { x: 0.61, y: 0.255 });
            await delay(200);
            await clickRatio({ x: 0.775, y: 0.523 });
            await delay(500);
            return true;
        }
        return false;
    ...
    const handleUnderfilledTeam = async () => {
        ...
        if (!findItem(current.result.items, ["可出战角色人数未达上限", "是否确认出战", "本局不再提示"], settings.buttonFuzzyScore))
            return;
        await clickRatio({ x: 0.463, y: 0.56 });
        await delay(200);
        await clickRatio({ x: 0.612, y: 0.622 });
        await delay(500);
    };
    const ensureAutoBattle = async (single = false) => {
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
            ...
            const detection = await (0, image_detectors_1.detectAutoBattleDisabled)(capture.base64);
    ...
    const clickBattleButton = async (battleCount) => {
    ...
    const runOpeningBattles = async () => {
        ...
        for (let cycle = 0; cycle < 300 && continueCount < 2; cycle++) {
            ...
            if (battleCount > continueCount)
                await ensureAutoBattle(true);
            if (await handleRoleChoice(current))
                continue;
            ...
            if (continued) {
                ...
                if ((0, text_matcher_1.fuzzyContains)(continued.alias, "继续挑战", 100))
                    continueCount++;
                await delay(200);
                continue;
            }
    ...
    const tryTargetStrategy = async (attempts) => {
        let best = null;
        for (let attempt = 0; attempt < attempts; attempt++) {
            ...
            if (found) {
                ...
                if (!best || priority < best.priority)
                    best = { ...found, capture: current.capture, priority };
                if (priority === 0)
                    break;
            }
    ...
    const runStrategyRecognition = async (allowExtraRefresh) => {
        for (let attempt = 0; attempt < 30; attempt++) {
            const current = await scan();
            if (findItem(current.result.items, STRATEGY_SCREEN_ALIASES, 85))
                break;
            if (attempt === 29)
                return null;
            await delay(1000);
        ...
        if (hit)
            return hit;
        for (let index = 0; index < exports.STRATEGY_REFRESH_POINTS.length; index++) {
            await clickRatio(exports.STRATEGY_REFRESH_POINTS[index]);
            if (index < exports.STRATEGY_REFRESH_POINTS.length - 1)
                await delay(200);
        }
        await delay(500);
        ...
        for (const item of current.result.items) {
            if (STRATEGY_BLACKLIST.some((word) => (0, text_matcher_1.fuzzyContains)(item.text, word, 85))) {
                blocked.add(Math.max(0, Math.min(2, Math.floor((item.bounds.x + item.bounds.width / 2) / (1000 / 3)))));
            }
        }
        const full = await captureOnly();
        const scores = await (0, image_detectors_1.collectionMarkerScores)(full.base64, CARD_REGIONS);
    ...
    const scanDebuffPage = async () => {
        ...
        for (let attempt = 0; attempt < DEBUFF_RECHECK_ATTEMPTS; attempt++) {
            ...
            const targets = (0, text_matcher_1.matchWords)(settings.debuffEnabled ? settings.targetWords : [], latest.result.rawText, settings.fuzzyScore);
            const blocked = (0, text_matcher_1.matchWords)(settings.blockedEnabled ? settings.blockedWords : [], latest.result.rawText, settings.blockedFuzzyScore);
            ...
            const pageReady = DEBUFF_SCREEN_HINTS.some((hint) => (0, text_matcher_1.fuzzyContains)(latest.result.rawText, hint, settings.buttonFuzzyScore));
            if (blocked.hits.length > 0 || targetSatisfied || pageReady)
                return latest;
            if (attempt + 1 < DEBUFF_RECHECK_ATTEMPTS)
                await delay(DEBUFF_RECHECK_INTERVAL_MS);
        }
        return latest;
    };
    const waitForOpeningBoard = async () => {
        for (let attempt = 0; attempt < 50; attempt++) {
            ...
            if (findItem(current.result.items, ["备战阶段", "出战"], settings.buttonFuzzyScore)
                || ["备战阶段", "出战"].some((word) => (0, text_matcher_1.fuzzyContains)(current.result.rawText, word, settings.buttonFuzzyScore))) {
                return;
            }
            if (attempt < 49)
                await delay(600);
    ...
    const returnToCurrencyWars = async () => {
        ...
        for (let attempt = 0; attempt < 54; attempt++) {
            ...
            if (findItem(current.result.items, ["下一步"], settings.buttonFuzzyScore)
                || (0, text_matcher_1.fuzzyContains)(current.result.rawText, "下一步", settings.buttonFuzzyScore))
                break;
            if (attempt < 53)
                await delay(150);
        }
        ...
        for (let attempt = 0; attempt < 14; attempt++) {
            ...
            if (findItem(current.result.items, ["开始货币战争"], settings.buttonFuzzyScore)
                || (0, text_matcher_1.fuzzyContains)(current.result.rawText, "开始货币战争", settings.buttonFuzzyScore))
                return;
            if (["当前进度", "继续进度", "结束并结算"]
                .some((word) => (0, text_matcher_1.fuzzyContains)(current.result.rawText, word, settings.buttonFuzzyScore))) {
    ...
    try {
        ...
        if (settings.recognitionOnly) {
            ...
            progress("只识别测试");
            const current = await scan();
            const targets = (0, text_matcher_1.matchWords)(settings.debuffEnabled ? settings.targetWords : [], current.result.rawText, settings.fuzzyScore).hits;
            const blocked = (0, text_matcher_1.matchWords)(settings.blockedEnabled ? settings.blockedWords : [], current.result.rawText, settings.blockedFuzzyScore).hits;
            const summary = [
        ...
        while (!settings.maxRounds || rounds < settings.maxRounds) {
            ...
            for (let index = 0; index < openingClicks; index++) {
                ...
                if (index + 1 < openingClicks)
                    await delay(OPENING_ADVANCE_INTERVAL_MS);
            }
            const openingSpan = (openingClicks - 1) * OPENING_ADVANCE_INTERVAL_MS;
            if (openingSpan < OPENING_ADVANCE_DURATION_MS)
                await delay(OPENING_ADVANCE_DURATION_MS - openingSpan);
            progress("识别局外词条");
            const debuff = await scanDebuffPage();
            const blocked = (0, text_matcher_1.matchWords)(settings.blockedEnabled ? settings.blockedWords : [], debuff.result.rawText, settings.blockedFuzzyScore);
            const targets = (0, text_matcher_1.matchWords)(settings.debuffEnabled ? settings.targetWords : [], debuff.result.rawText, settings.fuzzyScore);
            const targetSatisfied = settings.debuffEnabled && settings.targetWords.length > 0
                && (settings.targetMatchAny ? targets.hits.length > 0 : targets.missing.length === 0);
            ...
            if (canCheckInvestment) {
                ...
                for (let attempt = 0; attempt < 3; attempt++) {
                    ...
                    const hits = (0, text_matcher_1.matchWords)(investmentAliases, investment.result.rawText, settings.investmentFuzzyScore).hits;
                    if (hits.length > 0) {
            ...
            for (let attempt = 0; attempt < 4; attempt++) {
                if (await handleRoleChoice(await scan(DIALOG)))
                    break;
                if (attempt < 3)
                    await delay(300);
            }
            ...
            const extraRefresh = investmentHit !== null
                && ["银·金·彩", "银金彩", "银 金 彩"].some((word) => (0, text_matcher_1.fuzzyContains)(investmentHit, word, 100));
            const strategyHit = await runStrategyRecognition(extraRefresh);
            if (strategyHit)
                return { ok: true, rounds, matched: "局内策略：" + strategyHit };
            await returnToCurrencyWars();
        }
        ...
        if (err instanceof StopError)
            return { ok: false, rounds, error: "已中止" };
