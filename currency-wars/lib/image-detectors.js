...
exports.detectAutoBattleDisabled = detectAutoBattleDisabled;
exports.collectionMarkerScores = collectionMarkerScores;
...
async function gray(base64, width, height) {
    let pipeline = (0, sharp_1.default)(Buffer.from(base64, "base64"));
    if (width && height)
...
function ncc(source, template, left, top, sampleStep) {
    let ss = 0;
    let ts = 0;
    let ss2 = 0;
    let ts2 = 0;
...
function best(source, template, region, scanStep, sampleStep) {
    ...
    for (let y = top; y <= bottom; y += scanStep) {
        for (let x = left; x <= right; x += scanStep)
            score = Math.max(score, ncc(source, template, x, y, sampleStep));
    }
    return score;
}
async function detectAutoBattleDisabled(base64) {
...
async function collectionMarkerScores(base64, regions) {
...
//# sourceMappingURL=image-detectors.js.map